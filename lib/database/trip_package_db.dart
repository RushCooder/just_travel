

import '../models/trip_package_model.dart';

List<TripPackageModel> tripPackages = [
  TripPackageModel(
    id: 1,
    title: 'Cox\'s Bazar Beach',
    location: 'Cox\'s Bazar',
    availableDate: '21Fb-27Feb',
    price: 6000,
    images: [
      'https://1.bp.blogspot.com/-ysovcxw_Szw/YFhGajIirkI/AAAAAAAAFg8/od4MdJcD0sYPBqkd3KWeTGxCLtmfxlD1ACLcBGAsYHQ/s575/coxsbaza-development.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/10/e2/f8/43/longest-sea-beach-in.jpg?w=700&h=-1&s=1',
      'https://media.istockphoto.com/photos/multi-color-wooden-fishing-boat-on-the-shore-in-cox-bazar-sea-beach-picture-id1167217673?k=20&m=1167217673&s=612x612&w=0&h=84ljCOSeyV65glAWGcnDARP4l_eGN8HAOz2m8JUkuyI=',
    ],
    description:
        '''Cox’s Bazar is a town on the southeast coast of Bangladesh. It’s known for its very long, sandy beachfront, stretching from Sea Beach in the north to Kolatoli Beach in the south. Aggameda Khyang monastery is home to bronze statues and centuries-old Buddhist manuscripts. South of town, the tropical rainforest of Himchari National Park has waterfalls and many birds. North, sea turtles breed on nearby Sonadia Island''',
  ),
  TripPackageModel(
    id: 2,
    title: 'Sajek Tripuri Valley',
    location: 'Rangamati ',
    availableDate: '21Mar-27Mar',
    price: 6000,
    images: [
      'https://avijatrik.org/wp-content/uploads/2019/06/cover22-870x555.jpg',
      'https://dailyasianage.com/library/1576957938_5.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/09/bd/d1/fc/sajek.jpg?w=1200&h=1200&s=1',
    ],
    description:
        'Sajek Tripuri Valley is one of the most popular tourist spots in Bangladesh situated among the hills of the Kasalong range of mountains in Sajek union, Baghaichhari Upazila in Rangamati District.[3] The valley is 2,000 feet (610 m) above sea level.[4] Sajek Tripuri Valley is known as the Queen of Hills & Roof of Rangamati.',
  ),
  TripPackageModel(
    id: 3,
    title: 'Belaichari',
    location: 'Rangamati ',
    availableDate: '21Apr-27Apr',
    price: 6000,
    images: [
      'https://i0.wp.com/adarbepari.com/wp-content/uploads/2016/09/muppachora-waterfall-belaichari-rangamati-01.jpg?fit=820%2C430&ssl=1',
      'https://www.lrbtravelteam.com/wp-content/uploads/2020/11/Muppochora-Jhorna..png',
      'https://live.staticflickr.com/3854/32604350573_43928dce5f_b.jpg',
    ],
    description:
        '''Bilaichari is an Upazila which is a part of Rangamati district in south eastern Bangladesh. The Upazila is covered with waterfalls, hills, forests and lake''',
  ),
  TripPackageModel(
    id: 4,
    title: 'Tanguar Haor',
    location: 'Sunamganj  ',
    availableDate: '21May-27May',
    price: 6000,
    images: [
      'https://thefinancialexpress.com.bd/uploads/1630034469.jpg',
      'http://chokkor.com/wp-content/uploads/2018/10/Tanguar-Haor-1-960x630.jpg',
      'https://media-cdn.tripadvisor.com/media/photo-s/1b/a5/5f/e3/tanguar-hoar.jpg',
    ],
    description:
        '''Tanguar Haor (Bengali: টাঙ্গুয়ার হাওর; also called Tangua haor), located in the Dharmapasha and Tahirpur upazilas of Sunamganj District in Bangladesh, is a unique wetland ecosystem of national importance and has come into international focus. The area of Tanguar Haor including 46 villages within the haor is about 100 square kilometres (39 sq mi) of which 2,802.36 ha2 is wetland. It is the source of livelihood for more than 40,000 people. Bangladesh declared it an Ecologically Critical Area in 1999 considering its critical condition as a result of overexploitation of its natural resources''',
  ),TripPackageModel(
    id: 1,
    title: 'Cox\'s Bazar Beach',
    location: 'Cox\'s Bazar',
    availableDate: '21Fb-27Feb',
    price: 6000,
    images: [
      'https://1.bp.blogspot.com/-ysovcxw_Szw/YFhGajIirkI/AAAAAAAAFg8/od4MdJcD0sYPBqkd3KWeTGxCLtmfxlD1ACLcBGAsYHQ/s575/coxsbaza-development.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/10/e2/f8/43/longest-sea-beach-in.jpg?w=700&h=-1&s=1',
      'https://media.istockphoto.com/photos/multi-color-wooden-fishing-boat-on-the-shore-in-cox-bazar-sea-beach-picture-id1167217673?k=20&m=1167217673&s=612x612&w=0&h=84ljCOSeyV65glAWGcnDARP4l_eGN8HAOz2m8JUkuyI=',
    ],
    description:
        '''Cox’s Bazar is a town on the southeast coast of Bangladesh. It’s known for its very long, sandy beachfront, stretching from Sea Beach in the north to Kolatoli Beach in the south. Aggameda Khyang monastery is home to bronze statues and centuries-old Buddhist manuscripts. South of town, the tropical rainforest of Himchari National Park has waterfalls and many birds. North, sea turtles breed on nearby Sonadia Island''',
  ),
  TripPackageModel(
    id: 2,
    title: 'Sajek Tripuri Valley',
    location: 'Rangamati ',
    availableDate: '21Mar-27Mar',
    price: 6000,
    images: [
      'https://avijatrik.org/wp-content/uploads/2019/06/cover22-870x555.jpg',
      'https://dailyasianage.com/library/1576957938_5.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/09/bd/d1/fc/sajek.jpg?w=1200&h=1200&s=1',
    ],
    description:
        'Sajek Tripuri Valley is one of the most popular tourist spots in Bangladesh situated among the hills of the Kasalong range of mountains in Sajek union, Baghaichhari Upazila in Rangamati District.[3] The valley is 2,000 feet (610 m) above sea level.[4] Sajek Tripuri Valley is known as the Queen of Hills & Roof of Rangamati.',
  ),
  TripPackageModel(
    id: 3,
    title: 'Belaichari',
    location: 'Rangamati ',
    availableDate: '21Apr-27Apr',
    price: 6000,
    images: [
      'https://i0.wp.com/adarbepari.com/wp-content/uploads/2016/09/muppachora-waterfall-belaichari-rangamati-01.jpg?fit=820%2C430&ssl=1',
      'https://www.lrbtravelteam.com/wp-content/uploads/2020/11/Muppochora-Jhorna..png',
      'https://live.staticflickr.com/3854/32604350573_43928dce5f_b.jpg',
    ],
    description:
        '''Bilaichari is an Upazila which is a part of Rangamati district in south eastern Bangladesh. The Upazila is covered with waterfalls, hills, forests and lake''',
  ),
  TripPackageModel(
    id: 4,
    title: 'Tanguar Haor',
    location: 'Sunamganj  ',
    availableDate: '21May-27May',
    price: 6000,
    images: [
      'https://thefinancialexpress.com.bd/uploads/1630034469.jpg',
      'http://chokkor.com/wp-content/uploads/2018/10/Tanguar-Haor-1-960x630.jpg',
      'https://media-cdn.tripadvisor.com/media/photo-s/1b/a5/5f/e3/tanguar-hoar.jpg',
    ],
    description:
        '''Tanguar Haor (Bengali: টাঙ্গুয়ার হাওর; also called Tangua haor), located in the Dharmapasha and Tahirpur upazilas of Sunamganj District in Bangladesh, is a unique wetland ecosystem of national importance and has come into international focus. The area of Tanguar Haor including 46 villages within the haor is about 100 square kilometres (39 sq mi) of which 2,802.36 ha2 is wetland. It is the source of livelihood for more than 40,000 people. Bangladesh declared it an Ecologically Critical Area in 1999 considering its critical condition as a result of overexploitation of its natural resources''',
  ),TripPackageModel(
    id: 1,
    title: 'Cox\'s Bazar Beach',
    location: 'Cox\'s Bazar',
    availableDate: '21Fb-27Feb',
    price: 6000,
    images: [
      'https://1.bp.blogspot.com/-ysovcxw_Szw/YFhGajIirkI/AAAAAAAAFg8/od4MdJcD0sYPBqkd3KWeTGxCLtmfxlD1ACLcBGAsYHQ/s575/coxsbaza-development.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/10/e2/f8/43/longest-sea-beach-in.jpg?w=700&h=-1&s=1',
      'https://media.istockphoto.com/photos/multi-color-wooden-fishing-boat-on-the-shore-in-cox-bazar-sea-beach-picture-id1167217673?k=20&m=1167217673&s=612x612&w=0&h=84ljCOSeyV65glAWGcnDARP4l_eGN8HAOz2m8JUkuyI=',
    ],
    description:
        '''Cox’s Bazar is a town on the southeast coast of Bangladesh. It’s known for its very long, sandy beachfront, stretching from Sea Beach in the north to Kolatoli Beach in the south. Aggameda Khyang monastery is home to bronze statues and centuries-old Buddhist manuscripts. South of town, the tropical rainforest of Himchari National Park has waterfalls and many birds. North, sea turtles breed on nearby Sonadia Island''',
  ),
  TripPackageModel(
    id: 2,
    title: 'Sajek Tripuri Valley',
    location: 'Rangamati ',
    availableDate: '21Mar-27Mar',
    price: 6000,
    images: [
      'https://avijatrik.org/wp-content/uploads/2019/06/cover22-870x555.jpg',
      'https://dailyasianage.com/library/1576957938_5.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/09/bd/d1/fc/sajek.jpg?w=1200&h=1200&s=1',
    ],
    description:
        'Sajek Tripuri Valley is one of the most popular tourist spots in Bangladesh situated among the hills of the Kasalong range of mountains in Sajek union, Baghaichhari Upazila in Rangamati District.[3] The valley is 2,000 feet (610 m) above sea level.[4] Sajek Tripuri Valley is known as the Queen of Hills & Roof of Rangamati.',
  ),
  TripPackageModel(
    id: 3,
    title: 'Belaichari',
    location: 'Rangamati ',
    availableDate: '21Apr-27Apr',
    price: 6000,
    images: [
      'https://i0.wp.com/adarbepari.com/wp-content/uploads/2016/09/muppachora-waterfall-belaichari-rangamati-01.jpg?fit=820%2C430&ssl=1',
      'https://www.lrbtravelteam.com/wp-content/uploads/2020/11/Muppochora-Jhorna..png',
      'https://live.staticflickr.com/3854/32604350573_43928dce5f_b.jpg',
    ],
    description:
        '''Bilaichari is an Upazila which is a part of Rangamati district in south eastern Bangladesh. The Upazila is covered with waterfalls, hills, forests and lake''',
  ),
  TripPackageModel(
    id: 4,
    title: 'Tanguar Haor',
    location: 'Sunamganj  ',
    availableDate: '21May-27May',
    price: 6000,
    images: [
      'https://thefinancialexpress.com.bd/uploads/1630034469.jpg',
      'http://chokkor.com/wp-content/uploads/2018/10/Tanguar-Haor-1-960x630.jpg',
      'https://media-cdn.tripadvisor.com/media/photo-s/1b/a5/5f/e3/tanguar-hoar.jpg',
    ],
    description:
        '''Tanguar Haor (Bengali: টাঙ্গুয়ার হাওর; also called Tangua haor), located in the Dharmapasha and Tahirpur upazilas of Sunamganj District in Bangladesh, is a unique wetland ecosystem of national importance and has come into international focus. The area of Tanguar Haor including 46 villages within the haor is about 100 square kilometres (39 sq mi) of which 2,802.36 ha2 is wetland. It is the source of livelihood for more than 40,000 people. Bangladesh declared it an Ecologically Critical Area in 1999 considering its critical condition as a result of overexploitation of its natural resources''',
  ),TripPackageModel(
    id: 1,
    title: 'Cox\'s Bazar Beach',
    location: 'Cox\'s Bazar',
    availableDate: '21Fb-27Feb',
    price: 6000,
    images: [
      'https://1.bp.blogspot.com/-ysovcxw_Szw/YFhGajIirkI/AAAAAAAAFg8/od4MdJcD0sYPBqkd3KWeTGxCLtmfxlD1ACLcBGAsYHQ/s575/coxsbaza-development.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/10/e2/f8/43/longest-sea-beach-in.jpg?w=700&h=-1&s=1',
      'https://media.istockphoto.com/photos/multi-color-wooden-fishing-boat-on-the-shore-in-cox-bazar-sea-beach-picture-id1167217673?k=20&m=1167217673&s=612x612&w=0&h=84ljCOSeyV65glAWGcnDARP4l_eGN8HAOz2m8JUkuyI=',
    ],
    description:
        '''Cox’s Bazar is a town on the southeast coast of Bangladesh. It’s known for its very long, sandy beachfront, stretching from Sea Beach in the north to Kolatoli Beach in the south. Aggameda Khyang monastery is home to bronze statues and centuries-old Buddhist manuscripts. South of town, the tropical rainforest of Himchari National Park has waterfalls and many birds. North, sea turtles breed on nearby Sonadia Island''',
  ),
  TripPackageModel(
    id: 2,
    title: 'Sajek Tripuri Valley',
    location: 'Rangamati ',
    availableDate: '21Mar-27Mar',
    price: 6000,
    images: [
      'https://avijatrik.org/wp-content/uploads/2019/06/cover22-870x555.jpg',
      'https://dailyasianage.com/library/1576957938_5.jpg',
      'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/09/bd/d1/fc/sajek.jpg?w=1200&h=1200&s=1',
    ],
    description:
        'Sajek Tripuri Valley is one of the most popular tourist spots in Bangladesh situated among the hills of the Kasalong range of mountains in Sajek union, Baghaichhari Upazila in Rangamati District.[3] The valley is 2,000 feet (610 m) above sea level.[4] Sajek Tripuri Valley is known as the Queen of Hills & Roof of Rangamati.',
  ),
  TripPackageModel(
    id: 3,
    title: 'Belaichari',
    location: 'Rangamati ',
    availableDate: '21Apr-27Apr',
    price: 6000,
    images: [
      'https://i0.wp.com/adarbepari.com/wp-content/uploads/2016/09/muppachora-waterfall-belaichari-rangamati-01.jpg?fit=820%2C430&ssl=1',
      'https://www.lrbtravelteam.com/wp-content/uploads/2020/11/Muppochora-Jhorna..png',
      'https://live.staticflickr.com/3854/32604350573_43928dce5f_b.jpg',
    ],
    description:
        '''Bilaichari is an Upazila which is a part of Rangamati district in south eastern Bangladesh. The Upazila is covered with waterfalls, hills, forests and lake''',
  ),
  TripPackageModel(
    id: 4,
    title: 'Tanguar Haor',
    location: 'Sunamganj  ',
    availableDate: '21May-27May',
    price: 6000,
    images: [
      'https://thefinancialexpress.com.bd/uploads/1630034469.jpg',
      'http://chokkor.com/wp-content/uploads/2018/10/Tanguar-Haor-1-960x630.jpg',
      'https://media-cdn.tripadvisor.com/media/photo-s/1b/a5/5f/e3/tanguar-hoar.jpg',
    ],
    description:
        '''Tanguar Haor (Bengali: টাঙ্গুয়ার হাওর; also called Tangua haor), located in the Dharmapasha and Tahirpur upazilas of Sunamganj District in Bangladesh, is a unique wetland ecosystem of national importance and has come into international focus. The area of Tanguar Haor including 46 villages within the haor is about 100 square kilometres (39 sq mi) of which 2,802.36 ha2 is wetland. It is the source of livelihood for more than 40,000 people. Bangladesh declared it an Ecologically Critical Area in 1999 considering its critical condition as a result of overexploitation of its natural resources''',
  ),
];
